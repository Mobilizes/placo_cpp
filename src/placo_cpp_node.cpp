...
PlacoCppNode::PlacoCppNode(std::string urdf_path, std::string config_path_in)
: Node("placo_cpp_node")
{
  joints_pub_ =
    this->create_publisher<tachimawari_interfaces::msg::SetJoints>("/joint/set_joints", 10);
  ...
  footsteps =
    planner->plan(placo::humanoid::HumanoidRobot::Side::Left, T_world_left, T_world_right);
  ...
  // joint ID mapping
  joints_name = {{"head_yaw", 19}, {"head_pitch", 20}, {"left_shoulder_pitch", 2},
    {"left_shoulder_roll", 4}, {"left_elbow", 6}, {"right_shoulder_pitch", 1},
    {"right_shoulder_roll", 3}, {"right_elbow", 5}, {"left_hip_yaw", 8}, {"left_hip_roll", 10},
    {"left_hip_pitch", 12}, {"left_knee", 14}, {"left_ankle_pitch", 16}, {"left_ankle_roll", 18},
    {"right_hip_yaw", 7}, {"right_hip_roll", 9}, {"right_hip_pitch", 11}, {"right_knee", 13},
    {"right_ankle_pitch", 15}, {"right_ankle_roll", 17}};
...
void PlacoCppNode::load_configuration()
{
  ...
  if (jitsuyo::assign_val(config, "walking", walking_section)) {
    bool valid_section =
      jitsuyo::assign_val(walking_section, "single_support_duration", single_support_duration);
    valid_section &=
      jitsuyo::assign_val(walking_section, "single_support_timesteps", single_support_timesteps);
    valid_section &=
      jitsuyo::assign_val(walking_section, "double_support_ratio", double_support_ratio);
    valid_section &= jitsuyo::assign_val(
      walking_section, "startend_double_support_ratio", startend_double_support_ratio);
    ...
    valid_section &=
      jitsuyo::assign_val(walking_section, "walk_foot_rise_ratio", walk_foot_rise_ratio);
    ...
    valid_section &=
      jitsuyo::assign_val(walking_section, "walk_max_dx_forward", walk_max_dx_forward);
    valid_section &=
      jitsuyo::assign_val(walking_section, "walk_max_dx_backward", walk_max_dx_backward);
...
void PlacoCppNode::handleWalkTeleop()
{
  ...
  switch (key) {
    case 'w':
      walk_dx_ = std::min(walk_dx_ + speed_step, parameters.walk_max_dx_forward);
      break;
    case 's':
      walk_dx_ = std::max(walk_dx_ - speed_step, -parameters.walk_max_dx_backward);
      break;
    case 'j':
      walk_dy_ = std::min(walk_dy_ + speed_step, parameters.walk_max_dy);
      break;
    case 'l':
      walk_dy_ = std::max(walk_dy_ - speed_step, -parameters.walk_max_dy);
      break;
    case 'a':
      walk_dtheta_ = std::min(walk_dtheta_ + turn_step, parameters.walk_max_dtheta);
      break;
    case 'd':
      walk_dtheta_ = std::max(walk_dtheta_ - turn_step, -parameters.walk_max_dtheta);
      break;
    case ' ':
